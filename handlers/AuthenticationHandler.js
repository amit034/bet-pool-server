...
function handleLoginRequest(req, res) {
    ...
    if (password && username){
           ...
           performLogin(password,username).then(function(loginViewModel) {
                        // Return the login view model to the client
                        if(loginViewModel){
        ...
        res.json(401,{error :'Reason:  user name required'});
    }
}
// URL: /api/auth/logout
// POST parameter:
...
function performLogin(password,username) {
    var deferred = Q.defer();
        ...
        accountRepository.findAccountByUsernamePassword(username ,password)
            .then(function(account) {
                if (account){
                   ...
                   SecurityToken.findSecurityTokenFromUserId(account._id).then(function(securityToken){
                        if( securityToken){
