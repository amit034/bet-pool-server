    ...
    async bet(forOthers = false) {
        ...
        try {
            ...
            const openChallenge = await challengeRepository.findAllByQuery(
                openChallengeQuery, {
                include: [{model: Game, where: {eventId: {[Op.in]: eventsIds}, round: {[Op.gt]: 0}}, as: 'game'}],
                raw: true,
                transaction});
            const otherBets = await betRepository.findUserBetsByQuery({
                challengeId: {[Op.in]: _.map(openChallenge, 'id')}
            }, {transaction});
            const poolsChallenges = _.map(activePools, ({poolId, events, participates}) => {
                const challenges  = _.reduce(events, (aggEvent, event) => {
                    const eventChallenge = _.filter(openChallenge, ({game}) => game.eventId === event.id);
                    _.forEach(eventChallenge, ({id: challengeId}) => {
                        const alreadyBet = _.filter(otherBets, {challengeId});
                        const participantsToBet = !forOthers ? [this.id] : _.map(participates, 'userId');
                        const missingParticipantsBets = _.difference(participantsToBet, _.map(alreadyBet, 'userId'));
                        if(!_.isEmpty(missingParticipantsBets)) {
                            _.set(aggEvent, challengeId, missingParticipantsBets);
                        }
                    });
                    return aggEvent;
                }, {});
            ...
            const requiredChallenges = _.filter(openChallenge, ({id}) => _.some(poolsChallenges, ({challenges}) => _.has(challenges, id)));
            if(_.isEmpty(requiredChallenges)) return;
            const myBets = await betRepository.findUserBetsByQuery({userId: this.id, challengeId: {[Op.in]: _.map(requiredChallenges, 'id')}},
                {transaction});
            const learningData = await this.learningData(requiredChallenges, {transaction});
            const bets = await this.setBet({openChallenge: requiredChallenges, learningData});
            const betsByChallengeId = _.keyBy(bets, 'challengeId');

            const spreadBets = _.reduce(poolsChallenges, (agg, {poolId, challenges}) => {
                const betsMade =_.reduce(challenges, (aggChallenge, participants, challengeId) => {
                    const bet = betsByChallengeId[challengeId];
                    if (bet) {
                        const {score1, score2} = bet;
                        const participantsBets = _.map(participants, (userId) => {
                            return {poolId, challengeId, userId, score1, score2, isPublic: 1};
                        });
